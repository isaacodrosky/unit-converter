...
convertBtn.addEventListener("click", function () {
  ...
  volumeEl.textContent = `
        ${userInput} liters = ${(userInput * litersToGallons).toFixed(
    3
  )} gallons | 
        ${userInput} gallons = ${(userInput / litersToGallons).toFixed(
    3
  )} liters`;
  massEl.textContent = `
        ${userInput} kilograms = ${(userInput * kilogramsToPounds).toFixed(
    3
  )} pounds | 
        ${userInput} pounds = ${(userInput / kilogramsToPounds).toFixed(
    3
  )} kilograms`;
});
